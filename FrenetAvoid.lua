...
local function collectNearbyOpponents(egoProgressZ, allCars, outList)
  local count = 0
  for i = 1, #allCars do
    local tc = ac.worldCoordinateToTrack(allCars[i].position)  -- (x=offsetN, z=progressZ)
    if tc then
...
function FrenetAvoid.computeOffset(allCars, ego, dt)
  if not ego or not ac.hasTrackSpline() then return 0.0 end
  ...
  -- Build a tiny working set of opponents near us in progress
  collectNearbyOpponents(egoProgressZ, allCars, scratch_NearbyCars)
...
function FrenetAvoid.debugDraw(carIndex)
  if #scratch_CandidatePaths == 0 then
    Logger.log("FrenetAvoid.debugDraw: no candidate paths to draw")
    return
    end
  ...
  for j = 1, #bp.samples - 1 do
    render.debugLine(bp.samples[j].pos, bp.samples[j+1].pos, color_BestCandidate)--, 3.5)
  end
  ...
  if ego then
    ...
    render.debugLine(p1, p2, rgbm(1,1,1,1))--, 2.0)
  end
end
