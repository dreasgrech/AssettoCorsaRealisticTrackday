...
local stringValues = {
    ...
    [stringNames.TargetSideBlocked] = "Target side blocked by another car (%s) so not driving to the side",
    [stringNames.CarInBlindSpot] = 'Car in blind spot so not driving to the side',
    [stringNames.OvertakingCarInPits] = 'Car behind is in pits so not yielding',
    [stringNames.OvertakingCarTooFarBehind] = 'Overtaking car too far behind',
    [stringNames.OvertakingCarNotBehind] = 'Overtaking car not behind (clear) so not yielding',
    [stringNames.OvertakingCarBelowMinimumSpeed] = 'Overtaking car below minimum speed so not yielding',
    [stringNames.YieldingCarBelowMinimumSpeed] = 'Yielding car speed too low (corner/traffic) so not yielding',
    [stringNames.WeAreFasterThanOvertakingCar] = 'We are faster than the car behind so not yielding',
    [stringNames.OvertakingCarNotOnOvertakingSide] = 'Overtaking car not on overtaking lane so not yielding',
}
