...
A key unique for each individual PC (uses serial numbers of processor and motherboard). Asynchronous version. Returned value is in binary,
 ...
 value is computed once sometime at the launch.
  ...
  1. `callback`: `fun(err: string?, response: binary?)?`
...
Computes SHA-256 checksum for given binary data. Very secure, but might be slow with large amounts of data. Data string can contain zero bytes.
  ...
  1. `data`: `binary|number|boolean|ac.FFIStruct`
...
Use `bit.tohex()` to turn result into a hex representation. Data string can contain zero bytes.
  ...
  - `integer`
## Function ac.checksumCRC32(data)
...
Sets a callback which will be called when replay activates, deactivates or jumps around when active (could be good, for things, to clear out particles).
  ...
  - `ac.Disposable`
## Function ac.onCarSwap(carIndex, callback)
...
Sets a callback which will be called when a car completes a lap. Should work for remote cars as well.
  ...
  2. `callback`: `fun(carIndex: integer, lapTime: integer, valid: boolean, cuts: integer, lapsCount: integer, splits: integer[], lineCrossTime: number)`
...
Sets a callback which will be called when setup is loaded or saved. Use `ac.INIConfig.currentSetup()` to load the setup.
  ...
  - `ac.Disposable`
## Function ac.onAssistsChanged(callback)
...
contacts, use `ac.areShapesColliding()`.
  ...
  - `boolean`
## Function ac.getCollidingShapesContacts(shape1, transform1, shape2, transform2, maxContacts)
Generate list of contacts between two shapes. Added in 0.3.0-preview238. A more advanced (and expensive) version of `ac.areShapesColliding()`.
 ...
 going to help with performance.
  ...
  5. `maxContacts`: `integer?` Maximum number of contacts to find. Can’t exceed 128. Default value: 1.
## Function ac.getCarNameByID(carID, withYearPostfix)
Get car name of a car with a given ID. If there is no such car installed, or we couldn’t find a name, returns ID in quotes.
  ...
  2. `withYearPostfix`: `boolean?` Default value: `false`.

  Returns:

  - `string`
## Function ac.getTrackNameByID(trackID)
...
Calculates file size in bytes. Returns -1 if there was an error.
  ...
  - `integer`
## Function io.creationTime(filename, setValue)
Returns creation time as number of seconds since 1970, or -1 if there was an error.
  ...
  2. `setValue`: `integer?` If set, override last access time with this, still returning the previous value.

  Returns:

  - `integer`
## Function io.lastWriteTime(filename, setValue)
Returns last write time as number of seconds since 1970, or -1 if there was an error.
  ...
  - `integer`
## Function io.lastAccessTime(filename, setValue)
...
Any error thrown by `fn()` will be raised and not captured, but `dispose()` will be called either way.
  ...
  3. `...`: `any` Arguments to pass to `fn` (added in 0.3.0-preview259).
...
- `mat4x4:getEuler()`
  ...
  Returns:
    ...
    1. `number` Heading angle in radians.

    2. `number` Pitch angle in radians.

    3. `number` Roll angle in radians.
...
Encodes URL argument.
  ...
  - `string`
## Function string.urlDecode(self, plusForSpaces)
...
Xor two strings together. Works fastest if length of `key` is a multiple of 8. Input and output strings can contain zero bytes.
  ...
  1. `self`: `string` String to XOR.

  2. `key`: `string` Xoring key.
...
Compares a string against CSP-style filter (using “?” for “any characters”).
  ...
  - `boolean`
## Function string.urlParse(self, plusForSpaces)
...
If callback returns two values, second value is used as a key.
  ...
  - `T`
## Function table.build(iterator, state, nextKey)
...
# Module common/coroutine.lua

## Function coroutine.create(f, flags)
Creates a new coroutine, with body `f`. `f` must be a function. Returns this new coroutine, an object with type `thread`.
  ...
  2. `flags`: `nil|'e'|'s'|'es'` Flags: use `e` to raise errors from coroutine, `s` to start the coroutine immediately, or `es` for both.
...
Sleep for some time.
  ...
  1. `time`: `number` Time to sleep in seconds.
...
Ray collider. Added in 0.3.0-preview121.
  ...
  - `physics.ColliderType`
## Function physics.Collider.Track()
...
## Class WebResponse
## Function web.timeouts(resolve, connect, send, receive, threads)
...
function (https://learn.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpsettimeouts).
  ...
  5. `threads`: `integer?` If set, overrides maximum number of simultaneous requests. Default limit: 2. Maximum allowed value is 32.
...
## Function ui.GIFPlayer(source, simTime)
GIF player can be used to display animated GIFs. Also supports regular and animated WEBP images.
  ...
  2. `simTime`: `boolean?` Pass `true` to use simulation time (pause GIF when AC is paused), or `false` to use UI time. By default, `true` for car and track scripts.
...
CPU usage. Using this method on a shared player (if it’s played a bit) will disconnect the player and make it its own thing.
  ...
  Returns:
    ...
    - `ui.GIFPlayer` Returns itself for chaining several methods together.
...
- `ui.GIFPlayer:linkToSimulationSpeed(value)`
...
- `ac.SceneReference:setAnimation(filename, position, force)`
  ...
  Parameters:
    ...
    3. `force`: `boolean?` If not set to `true`, animation will be applied only if animation position is different from position used previously. Default value: `false`. You might want to use this flag, for example, if some other part of logic might have moved objects and you want to restore their positions as defined by the animation.

  Returns:

    - `self` Returns self for easy chaining.

- `ac.SceneReference:blendAnimation(filename, position, mix, normalize)`
...
To use remote animations, first load them with `web.loadRemoteAnimation()`.
  ...
  Parameters:
    ...
    3. `mix`: `number?` How much this animation should contribute. Default value: `1`. Passing a value equals to or below 0 is the same as not calling the function at all.

    4. `normalize`: `boolean?` Normalize transforms of all the nodes affected by this animation. Resets scale to 100%, removes any skewing.
...
```
  ...
  Parameters:
    ...
    6. `culling`: `integer?` Set to -1 to disable backface culling, or to 0 to hit backfaces only. Default value: 1.
...
- `ac.GeometryShot:setGrass(value)`

  Enables grass in the shot. By default, grass is not drawn. Careful: at the moment, grass exists only around the camera.

  Parameters:

    1. `value`: `'dedicated'|boolean?` Set to `true` to enable grass. Default value: `true`. Since 0.3.0-preview307, you can pass `'dedicated'` to set an alternative Grass FX spawn area (expensive).
...
- `ac.GeometryShot:setAlternativeShadowsSet(type)`
  ...
  Parameters:
    ...
    1. `type`: `'area'|'dedicated'` Type of shadow set to use: `area` is a basic shadow map around the main camera (without directions), `dedicated` is separate set of shadows for this geometry shot (expensive).
...
## Function physics.setPenalties(penalties)
Toggle penalties on and off. Added in 0.3.0-preview243. New Mode scripts can use this function even if the rest of Physics API is not available.
  ...
  1. `penalties`: `boolean|`true`|`false``
## Function physics.setWrongWayPenalty(penalty)
Toggle wrong way penalty on and off. Added in 0.3.0-preview243. New Mode scripts can use this function even if the rest of Physics API is not available.
  ...
  1. `penalty`: `boolean|`true`|`false``
## Function physics.setAssists(assists, targetCarIndex)
Set assists. Added in 0.3.0-preview254. New Mode scripts can use this function even if the rest of Physics API is not available. If a field is not set, original value will remain.
 ...
 Set `editable` to `false` to disallow editing assists from the main menu.
  ...
  1. `assists`: `{autoBlip: boolean?, autoClutch: boolean?, autoShifter: boolean?, abs: 'off'|'factory'|'on'?, tractionControl: 'off'|'factory'|'on'?, stability: number?, damageRate: number?, fuelRate: number?, tyresWear: number?, tyresBlankets: boolean?, slipstream: number?, idealLine: boolean?, editable: boolean?}`

  2. `targetCarIndex`: `integer?` Target car index. If non-zero, global assists will be ignored. Per-car assists: auto-blip, auto-clutch, auto-shift, ABS, TC, stability control, slipstream. Default value: 0.

  Returns:

  - `integer` Return number of changed items.
...
- `ac.AppWindowAccessor:redirectLayer2()`

  Returns redirect layer, or 0 if redirect is disabled. Redirected apps can be accessed via `dynamic::hud::redirected::N` textures.

  Returns:

    1. `integer` 0 if redirect is disabled.

    2. `boolean` `true` if window is duplicated to that layer. Before 0.3.0-preview285, this value would be `nil` by mistake.
...
## Function render.glTexture(filename)
  ...
  1. `filename`: `string`
## Function render.circle(pos, dir, radius, color, borderColor, useWhiteRefPoint)
Draws a simple circle othrogonal to `dir` direction using an optimized shader.
  ...
  6. `useWhiteRefPoint`: `boolean?` Set to `true` if you want white color to appear white. Default value: `false`.
## Function render.rectangle(pos, dir, width, height, color, useWhiteRefPoint)
Draws a simple rectangle othrogonal to `dir` direction using an optimized shader.
  ...
  6. `useWhiteRefPoint`: `boolean?` Set to `true` if you want white color to appear white. Default value: `false`.
## Function render.quad(p1, p2, p3, p4, color, texture, useWhiteRefPoint)
Draws a simple quad from four points.
  ...
  7. `useWhiteRefPoint`: `boolean?` Set to `true` if you want white color to appear white. Default value: `false`.
...
Called when AC is drawing something.
 ...
 - `main.hud.vr`: if first argument is `true`, HUD is rendered after post-processing (or post-processing is disabled), so things you draw have to be in 0…1 space. Return `true` to hide original HUD.
  ...
  1. `eventName`: `'main.root.opaque'|'main.track.opaque'|'main.root.transparent'|'main.track.transparent'|'main.smoke'|'mirror.track.opaque'|'mirror.root.transparent'|'shadow.root'|'main.hud.vr'`
...
Is current window pinned.
  ...
  - `boolean`
## Function ui.setWindowMouseScrollAxis(horizontal)

  Parameters:

  1. `horizontal`: `boolean|`true`|`false``
...
Add a dummy item of given size. unlike `ui.invisibleButton()`, dummy won’t take the mouse click or be navigable into.
  ...
  1. `size`: `number|vec2`
## Function ui.thinScrollbarBegin(appearOnHover, horizontal)
Returns `true` during scrolling.
  ...
  2. `horizontal`: `boolean?` Added in 0.3.0-preview238. Default value: `false`.
...
Since 0.2.8, return `'finalize'` to allow `ui.popup()` and `ui.onUIFinale()`.
  ...
  - `ac.Disposable`
## Function ui.modalPopup(title, msg, okText, cancelText, okIconID, cancelIconID, callback, autoclose)
...
with a single button only.
  ...
  8. `autoclose`: `boolean?` Automatically close if clicked outside of dialog, or if Escape button is pressed. Default value: `false`.
...
## Function ui.modalPrompt(title, msg, defaultValue, okText, cancelText, okIconID, cancelIconID, callback, autoclose)
Open modal popup message with text input, OK and Cancel buttons, return user choice via callback.
  ...
  9. `autoclose`: `boolean?` Automatically close if clicked outside of dialog, or if Escape button is pressed. Default value: `false`.
...
Returns `true` if script can affect physics engine. If not, only `physics.raycastTrack()` is available.
  ...
  - `boolean`
## Function physics.raycastTrack(pos, dir, length, point, normal, includeNoise)
...
Note: smaller rays are faster to cast. Also, rays going straight down are slightly faster as well.
  ...
  6. `includeNoise`: `boolean?` Set to `true` to take procedural sine noise into account. Default value: `false`.
...
Controls if AI goes to pits or not. Since 0.3.0-preview147, calling this function with `false` for value blocks out AI going to pits for a short time too.
  ...
  2. `value`: `boolean?` Default value: `true`.
## Function physics.setAILapsToComplete(carIndex, value)
...
## Function ac.setDriverShiftAnimationPreloadThreshold(carIndex, threshold)
...
Sets a callback which will be called when drawing driver context menu. Within, `ui.` functions can be used to add additional information.
  ...
  - `ac.Disposable`
## Function ac.forceDetailedCarLOD(carIndex)
...
Load car state. Available in few basic modes in offline only (such as practice with a single car), invalidates lap.
  ...
  2. `dataNext`: `binary|number|boolean|ac.FFIStruct|nil` Default value: `nil`.
...
## Function ac.resetDynamicTrackObjects(randomSeed, timePassed)
Reset dynamic track objects. Usually, AC does something similar when a new session starts. Objects will be randomly shifted based on `random_seed`.
  ...
  1. `randomSeed`: `integer?` If set, this random seed will be used to randomize objects positioning.

  2. `timePassed`: `number?` If above zero, objects will be moved along their velocities using this value as time in seconds. Default value: 0.
...
Not available to scripts without I/O access.
  ...
  1. `active`: `boolean?` Default value: `true`.
## Function ac.debugTrackColliders(filter, maxDistance)
Enables and disables track colliders debug view.
Not available to scripts without I/O access.

  Parameters:

  1. `filter`: `boolean|string|nil` Filter specifying which objects to show. Pass `false` to disable debugging, or `true` to show all.

  2. `maxDistance`: `number?` Default value: 1000.
...
Not available to scripts without I/O access.
  ...
  2. `data`: `string?` List of trees in that text file format, or `nil` to clear out the list.
...
Mixes up coordinates (comparing to functions like `ac.getTrackCoordinatesDeg()`)! Use fixed `ac.setTrackCoordinatesDeg2()` instead.
  ...
  1. `coords`: `number|vec2` X for longitude, Y for latitude (accidental mix up).

  2. `headingDeg`: `number?` Heading angle in degrees, optional. Default value: `nil`.
## Function ac.setTrackCoordinatesDeg2(coords, headingDeg)
...
Not available to scripts without I/O access.
  ...
  1. `coords`: `number|vec2` X for latitude, Y for longitude.
...
Packs driver model into a replacement package.
  ...
  2. `lodAData`: `binary|number|boolean|ac.FFIStruct`

  3. `lodBData`: `binary|number|boolean|ac.FFIStruct`

  4. `configData`: `binary|number|boolean|ac.FFIStruct`
...
Not available to scripts without I/O access.
  ...
  1. `config`: `'controls.ini'|'video.ini'|'gameplay.ini'|'assetto_corsa.ini'|'mouse_hider.ini'|'proximity_indicator.ini'|'graphics.ini'|'graphics_adjustments.ini'|'custom_rendering_modes.ini'|'general.ini'|'walking_out.ini'` Config file name, not the full path.
...
Runs asynchronously.
  ...
  1. `filename`: `string?` If not set, default path will be used.

  2. `format`: `{format: ac.ScreenshotFormat?, accumulation: boolean?}|ac.ScreenshotFormat|nil` Default value: `ac.ScreenshotFormat.Auto`.
...
Changes behavior of smoke going off a tyre (won’t affect dust or water splashes). Actual movement of smoke might change with CSP updates.
  ...
  2. `intensity`: `number` Use `math.nan` to restore original behavior, or a value from 0 to 1.
...
## Function ac.setGearLabel(c, disableShiftingAnimation)
...
using `ac.getCar().gearLabel`.
  ...
  2. `disableShiftingAnimation`: `boolean?` Pass `true` to stop any shifting animations. Default value: `false`.
...
Use `ac.setTractionControl()` instead.
  ...
  1. `mode`: `integer`
## Function ac.setTractionControl(mode)

  Parameters:

  1. `mode`: `integer`
## Function ac.setAutoShifter(active)
...
Loads car setup from a file. Works only from setup menu, and only if setup is not fixed.
  ...
  1. `setupFilename`: `binary|number|boolean|ac.FFIStruct` Absolute path, or path relative to “Documents\Assetto Corsa\setups\<car ID>”. Since CSP v0.1.80-preview459 can also be setup contents.
...
## Function ac.sendKeyboardButtonEvent(keyIndex, down)
...
Create a new audio event from previously loaded soundbank.
  ...
  1. `event`: `string|{event: string, dsp: ac.AudioDSP[]?}` Event name, for example, `'/cars/lada_revolution/door'` (leading “/” or “event:” prefix are optional). Since 0.3.0-preview147, you can use `:own` or `:0` (0-based car index) instead of car ID, so your logic could work if car uses sound from a different car. For example, `'cars/:own/engine_ext'` or `'cars/:1/engine_int'`. Remember, when writing CSP car config for a car you can use `[AUDIO_REDIRECT] CARS=primary_car_id, secondary_candidate` to use audio from existing car instead (the first one found would be used), or `[AUDIO_REDIRECT] SHARED=…` to load one of the shared remote audiobanks.
...
in Render Stats app.
  ...
  3. `...`: `any[]` Arguments to pass to `drawFn`. Added in 0.3.0-preview259.
...
- `ray:track(culling)`
  ...
  Parameters:
    ...
    1. `culling`: `integer?` Set to -1 to disable backface culling, or to 0 to hit backfaces only. Default value: 1.
...
- `ray:physics(outPosition, outNormal, includeNoise)`
  ...
  Parameters:
    ...
    3. `includeNoise`: `boolean?` Set to `true` to take procedural sine noise into account.
